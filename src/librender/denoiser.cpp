...
MTS_NAMESPACE_BEGIN
	...
	void Denoiser::init(const Vector2i filmSize, const bool filterFeatures, const bool filterVariance) {
		m_filterFeatures = filterFeatures;
		m_filterVariance = filterVariance;
		...
		m_filterVarianceColor = std::unique_ptr<Vector3[]>(new Vector3[numPixels]);
		m_filterVarianceOutput = std::unique_ptr<Vector3[]>(new Vector3[numPixels]);
		...
		m_bufferVarianceOutput = m_oidnDevice.newBuffer(numPixels * 3 * sizeof(float));
		m_bufferVarianceColor = m_oidnDevice.newBuffer(numPixels * 3 * sizeof(float));
		...
		if (m_filterFeatures){
			...
			if(m_filterVariance) {
				m_oidnVarianceFilter = m_oidnDevice.newFilter("RT");
				m_oidnVarianceFilter.setImage("color", m_bufferVarianceColor, oidn::Format::Float3, filmSize[0], filmSize[1]);
				m_oidnVarianceFilter.setImage("albedo", m_bufferAlbedoOutput, oidn::Format::Float3, filmSize[0], filmSize[1]);
				m_oidnVarianceFilter.setImage("normal", m_bufferNormalOutput, oidn::Format::Float3, filmSize[0], filmSize[1]);
				m_oidnVarianceFilter.setImage("output", m_bufferVarianceOutput, oidn::Format::Float3, filmSize[0], filmSize[1]);
				m_oidnVarianceFilter.set("cleanAux", true); // auxiliary images will be prefiltered
				m_oidnVarianceFilter.set("hdr", true);
				m_oidnVarianceFilter.commit();
			}
			...
			if(m_filterVariance) {
				m_oidnVarianceFilter = m_oidnDevice.newFilter("RT");
				m_oidnVarianceFilter.setImage("color", m_bufferVarianceColor, oidn::Format::Float3, filmSize[0], filmSize[1]);
				m_oidnVarianceFilter.setImage("albedo", m_bufferAlbedo, oidn::Format::Float3, filmSize[0], filmSize[1]);
				m_oidnVarianceFilter.setImage("normal", m_bufferNormal, oidn::Format::Float3, filmSize[0], filmSize[1]);
				m_oidnVarianceFilter.setImage("output", m_bufferVarianceOutput, oidn::Format::Float3, filmSize[0], filmSize[1]);
	...
	void Denoiser::denoise(){
		...
		if(m_filterVariance) {
			m_bufferVarianceColor.write(0, m_filmSize[0]*m_filmSize[1]*3*sizeof(float), &m_filterVarianceColor[0]);
		}
		...
		if(m_filterVariance) {
			m_bufferVarianceOutput.read(0, m_filmSize[0]*m_filmSize[1]*3*sizeof(float), &m_filterVarianceOutput[0]);
		}
	...
	void Denoiser::storeBuffers(std::string filename){
		...
		ref<Bitmap> bitmap_colorVariance  = new Bitmap(Bitmap::ERGB, Bitmap::EFloat32, m_filmSize, 3, (uint8_t*) m_bufferVarianceColor.getData());
		bitmap_colorVariance->setChannelNames({"color_variance.R", "color_variance.G", "color_variance.B"});
		ref<Bitmap> bitmap_outputVariance = new Bitmap(Bitmap::ERGB, Bitmap::EFloat32, m_filmSize, 3, (uint8_t*) m_bufferVarianceOutput.getData());
		bitmap_outputVariance->setChannelNames({"output_variance.R", "output_variance.G", "output_variance.B"});
		...
		if (m_filterFeatures){
			...
			if(m_filterVariance) {
				ref<Bitmap> bitmap = Bitmap::join(Bitmap::EMultiChannel, {bitmap_color, bitmap_colorVariance, bitmap_albedo, bitmap_albedo_output, bitmap_normal, bitmap_normal_output, bitmap_output, bitmap_outputVariance});
			...
			if(m_filterVariance) {
				ref<Bitmap> bitmap = Bitmap::join(Bitmap::EMultiChannel, {bitmap_color, bitmap_colorVariance, bitmap_albedo, bitmap_normal, bitmap_output, bitmap_outputVariance});
				bitmap->write(Bitmap::EOpenEXR, filename);
			} else {
	...
	void Denoiser::loadBuffers(std::string filename){
		...
		bitmap = new Bitmap(filename, "color_variance");
		m_bufferVarianceColor.write(0, size[0]*size[1]*3*sizeof(float), bitmap->getData());
		std::memcpy(&m_filterVarianceColor[0], bitmap->getData(), m_filmSize[0]*m_filmSize[1]*sizeof(Vector3));
		
		bitmap = new Bitmap(filename, "output_variance");
		m_bufferVarianceOutput.write(0, size[0]*size[1]*3*sizeof(float), bitmap->getData());
		std::memcpy(&m_filterVarianceOutput[0], bitmap->getData(), m_filmSize[0]*m_filmSize[1]*sizeof(Vector3));
	}
	...
	void Denoiser::add(int pixIdx, const Denoiser::Sample& sample){
		...
		if (m_filterVariance){
			m_filterVarianceColor[pixIdx] = (1.f - alpha) * m_filterVarianceColor[pixIdx] + alpha * Vector3(sample.color[0]*sample.color[0], sample.color[1]*sample.color[1], sample.color[2]*sample.color[2]);
		}
	}

	void Denoiser::addZeroColor(int pixIdx){
		m_sampleCounts[pixIdx] +=  1;
		float alpha = 1.f / m_sampleCounts[pixIdx];
		m_filterColor[pixIdx] = (1.f - alpha) * m_filterColor[pixIdx] + alpha * Vector3(0.f);
		if (m_filterVariance){
			m_filterVarianceColor[pixIdx] = (1.f - alpha) * m_filterVarianceColor[pixIdx] + alpha * Vector3(0.f);
		}
	}
